# Performance Lab — Rust Optimization Challenges

> 5 tasks · Total speedup: 103×

## 1. Поиск дубликатов в массиве 10M строк
**Category:** Алгоритмы · **Difficulty:** Advanced · **Speedup:** 2.3×

### Problem
Найти все дублирующиеся строки в массиве из 10 миллионов строк (средняя длина 32 символа). Задача требует оптимального баланса между скоростью и потреблением памяти.
...
|--------|----------|-----------|
| Time | 4.2s | 1.8s |
| Memory | 1800 MB | 640 MB |
| Speedup | — | 2.3× |
| Memory Delta | — | -64% |

### Complexity
- **Baseline:** Time O(n) avg, Space O(n)
- **Optimized:** Time O(n log n), Space O(1) extra
...
3. **Cache-local sort** — sort_unstable_by — unstable sort в 2x быстрее stable, работает на месте без extra аллокаций
...
**Category:** I/O · **Difficulty:** Expert · **Speedup:** 7.1×

### Problem
Парсинг CSV файла 500MB с 5M строками и 10 колонками. Необходимо извлечь только 3 конкретные колонки, минимизируя использование памяти и I/O операции.
...
|--------|----------|-----------|
| Time | 8.5s | 1.2s |
| Memory | 3200 MB | 512 MB |
| Speedup | — | 7.1× |
| Memory Delta | — | -84% |

### Complexity
- **Baseline:** Time O(n×m), Space O(n×m)
- **Optimized:** Time O(n), Space O(k) output only
...
**Category:** Конкурентность · **Difficulty:** Advanced · **Speedup:** 58.8×

### Problem
Выполнить 100K HTTP GET запросов к REST API, собрав все ответы. Необходимо максимизировать throughput при ограниченных ресурсах клиента.
...
### Complexity
- **Baseline:** Time O(n) sequential, Space O(k) results
- **Optimized:** Time O(n/c) c=concurrency, Space O(k) results
...
**Category:** Вычисления · **Difficulty:** Expert · **Speedup:** 8.4×

### Problem
Умножить две матрицы 1000×1000 (f32). Наивная реализация страдает от cache misses — необходимо оптимизировать доступ к памяти.
...
**Category:** Параллелизм · **Difficulty:** Expert · **Speedup:** 26.7×

### Problem
Реализовать bounded MPSC очередь: 8 producer threads, 1 consumer thread, 10M сообщений. Mutex-блокировки создают contention bottleneck.
...
### Complexity
- **Baseline:** Time O(n) + contention, Space O(n)
- **Optimized:** Time O(n) amortized, Space O(capacity)
